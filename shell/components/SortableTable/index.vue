  ...
  <div ref="container" :data-testid="componentTestid + '-list-container'">
    <div :class="{ 'titled': $slots.title && $slots.title.length }" class="sortable-table-header">
      ...
      <div v-if="showHeaderRow" class="fixed-header-actions"
        ...
        <div :class="bulkActionsClass" class="bulk">
          <slot name="header-left">
            <template v-if="tableActions">
              <button v-for="(act) in availableActions" :id="act.action" :key="act.action"
                v-clean-tooltip="actionTooltip" type="button" class="btn role-primary"
                :class="{ [bulkActionClass]: true }" :disabled="!act.enabled"
                :data-testid="componentTestid + '-' + act.action" role="button" :aria-label="act.label"
                @click="applyTableAction(act, null, $event)" @keydown.enter.stop
                @mouseover="setBulkActionOfInterest(act)" @mouseleave="setBulkActionOfInterest(null)">
                <i v-if="act.icon" :class="act.icon" />
                <span v-clean-html="act.label" />
              </button>
              <template v-if="featureDropdownMenu">
                <ActionDropdownShell :disabled="!selectedRows.length" :hidden-actions="hiddenActions"
                  :action-tooltip="actionTooltip" @click="applyTableAction" @mouseover="setBulkActionOfInterest"
                  @mouseleave="setBulkActionOfInterest" />
              </template>
              <template v-else>
                <ActionDropdown :class="bulkActionsDropdownClass" class="bulk-actions-dropdown"
                  :disable-button="!selectedRows.length" size="sm">
                  ...
                  <template #popover-content>
                    <ul class="list-unstyled menu">
                      <li v-for="(act, i) in hiddenActions" :key="i" v-close-popper v-clean-tooltip="{
                        content: actionTooltip,
                        placement: 'right'
                      }" :class="{ disabled: !act.enabled }" @click="applyTableAction(act, null, $event)"
                        @mouseover="setBulkActionOfInterest(act)" @mouseleave="setBulkActionOfInterest(null)">
                        <i v-if="act.icon" :class="act.icon" />
                        <span v-clean-html="act.label" />
                      </li>
        ...
        <div v-if="!hasAdvancedFiltering && $slots['header-middle']" class="middle">
          <slot name="header-middle" />
        </div>

        <div v-if="search || hasAdvancedFiltering || isTooManyItemsToAutoUpdate || $slots['header-right']"
          class="search row" data-testid="search-box-filter-row">
          ...
          <AsyncButton v-if="!hideManualRefreshButton && isTooManyItemsToAutoUpdate" mode="manual-refresh"
            :size="manualRefreshButtonSize" :current-phase="refreshButtonPhase" @click="debouncedRefreshTableData" />
          <div v-if="hasAdvancedFiltering" ref="advanced-filter-group" class="advanced-filter-group">
            ...
            <div v-show="advancedFilteringVisibility" class="advanced-filter-container">
              <input ref="advancedSearchQuery" v-model="advFilterSearchTerm" type="search" class="advanced-search-box"
                :placeholder="t('sortableTable.filterFor')">
              <div class="middle-block">
                <span>{{ t('sortableTable.in') }}</span>
                <LabeledSelect v-model:value="advFilterSelectedProp" class="filter-select" :clearable="true"
                  :options="advFilterSelectOptions" :disabled="false" :searchable="false" mode="edit" :multiple="false"
                  :taggable="false" :placeholder="t('sortableTable.selectCol')"
                  @selecting="(col) => advFilterSelectedLabel = col.label" />
              </div>
              <div class="bottom-block">
                <button class="btn role-secondary" :disabled="!advancedFilteringValues.length"
          ...
          <p v-else-if="search" id="describe-filter-sortable-table" hidden>
            {{ t('sortableTable.filteringDescription') }}
          </p>
          <input v-if="search" ref="searchQuery" v-model="eventualSearchQuery" type="search" class="input-sm search-box"
            :aria-label="t('sortableTable.searchLabel')" aria-describedby="describe-filter-sortable-table"
            :placeholder="t('sortableTable.search')">
          <slot name="header-button" />
        </div>
      </div>
    </div>
    <table ref="table" class="sortable-table" id="rounded-table" :class="classObject" width="100%" role="table">
      <THead v-if="showHeaders" :label-for="labelFor" :columns="columns" :group="group" :group-options="advGroupOptions"
        :has-advanced-filtering="hasAdvancedFiltering" :adv-filter-hide-labels-as-cols="advFilterHideLabelsAsCols"
        :table-actions="tableActions" :table-cols-options="columnOptions" :row-actions="rowActions"
        :sub-expand-column="subExpandColumn" :row-actions-width="rowActionsWidth" :how-much-selected="howMuchSelected"
        :sort-by="sortBy" :default-sort-by="_defaultSortBy" :descending="descending" :no-rows="noRows"
        :loading="isLoading && !loadingDelay" :no-results="noResults" @on-toggle-all="onToggleAll"
        @on-sort-change="changeSort" @col-visibility-change="changeColVisibility"
        @group-value-change="(val) => $emit('group-value-change', val)" @update-cols-options="updateColsOptions" />
      ...
      <!-- There are rows Matching  -->
      <tbody v-for="(groupedRows) in displayRows" v-else :key="groupedRows.key" tabindex="-1"
        :class="{ group: groupBy }">
        ...
        <template v-for="(row, i) in groupedRows.rows" :key="i">
          <slot name="main-row" :row="row.row">
            <slot :name="'main-row:' + (row.row.mainRowKey || i)" :full-colspan="fullColspan">
              ...
              <tr class="main-row" :data-testid="componentTestid + '-' + i + '-row'"
                :class="{ 'has-sub-row': row.showSubRow }" :data-node-id="row.key"
                ...
                <td v-if="tableActions" class="row-check" align="middle">
                  {{ row.mainRowKey }}
                  <Checkbox class="selection-checkbox" :data-node-id="row.key"
                    :data-testid="componentTestid + '-' + i + '-checkbox'" :value="selectedRows.includes(row.row)"
                ...
                <td v-if="subExpandColumn" class="row-expand" align="middle">
                  <i data-title="Toggle Expand" :class="{
                    icon: true,
                ...
                <template v-for="(col, j) in row.columns" :key="j">
                  <slot :name="'col:' + col.col.name" :row="row.row" :col="col.col" :dt="dt" :expanded="expanded"
                    :rowKey="row.key">
                    <td v-show="!hasAdvancedFiltering || (hasAdvancedFiltering && col.col.isColVisible)"
                      :key="col.col.name" :data-title="col.col.label" :data-testid="`sortable-cell-${i}-${j}`"
                      ...
                      <slot :name="'cell:' + col.col.name" :row="row.row" :col="col.col" :value="col.value">
                        <component :is="col.component" v-if="col.component && col.needRef" ref="column"
                          :value="col.value" :row="row.row" :col="col.col" :get-custom-detail-link="getCustomDetailLink"
                          v-bind="col.col.formatterOpts" :row-key="row.key" />
                        <component :is="col.component" v-else-if="col.component" :value="col.value" :row="row.row"
                          :col="col.col" v-bind="col.col.formatterOpts" :row-key="row.key" />
                        <component :is="col.col.formatter" v-else-if="col.col.formatter" :value="col.value"
                          :row="row.row" :col="col.col" v-bind="col.col.formatterOpts" :row-key="row.key" />
                ...
                <td v-if="rowActions">
                  <slot name="row-actions" :row="row.row" :index="i">
                    <template v-if="featureDropdownMenu">
                      <ActionMenu :resource="row.row" :data-testid="componentTestid + '-' + i + '-action-button'"
                        :button-aria-label="t('sortableTable.tableActionsLabel', { resource: row?.row?.id || '' })" />
                    </template>
                    <template v-else>
                      <ButtonMultiAction :id="`actionButton+${i}+${(row.row && row.row.name) ? row.row.name : ''}`"
                        :ref="`actionButton${i}`" aria-haspopup="true" aria-expanded="false"
                        :aria-label="t('sortableTable.tableActionsLabel', { resource: row?.row?.id || '' })"
                        :data-testid="componentTestid + '-' + i + '-action-button'" :borderless="true"
                        @click="handleActionButtonClick(i, $event)" @keyup.enter="handleActionButtonClick(i, $event)"
                        @keyup.space="handleActionButtonClick(i, $event)" />
                    </template>
                  </slot>
                </td>
              </tr>
              <!-- divider row, but not after the last row -->
              <tr v-if="i < groupedRows.rows.length - 1" class="row-divider">
                <td :colspan="fullColspan" style="text-align:center; padding:0;">
                  <hr style="width:95%; margin:0 auto; border:0; border-top:1px solid #0B1739;" />
                </td>
              </tr>
            </slot>
          </slot>
          <slot v-if="row.showSubRow" name="sub-row" :full-colspan="fullColspan" :row="row.row"
            :sub-matches="subMatches" :keyField="keyField" :componentTestid="componentTestid" :i="i"
            :onRowMouseEnter="onRowMouseEnter" :onRowMouseLeave="onRowMouseLeave">
            <tr v-if="row.row.stateDescription" :key="row.row[keyField] + '-description'"
              :data-testid="componentTestid + '-' + i + '-row-description'" class="state-description sub-row"
              @mouseenter="onRowMouseEnter" @mouseleave="onRowMouseLeave">
              <td v-if="tableActions" class="row-check" align="middle" />
    ...
    <div v-if="showPaging" class="paging">
      <button type="button" class="btn btn-sm role-multi-action" data-testid="pagination-first"
        :disabled="page == 1 || loading" role="button" :aria-label="t('sortableTable.ariaLabel.firstPageBtn')"
        @click="goToPage('first')">
        <i class="icon icon-chevron-beginning" :alt="t('sortableTable.alt.firstPageBtn')" />
      </button>
      <button type="button" class="btn btn-sm role-multi-action" data-testid="pagination-prev"
        :disabled="page == 1 || loading" role="button" :aria-label="t('sortableTable.ariaLabel.prevPageBtn')"
        @click="goToPage('prev')">
        <i class="icon icon-chevron-left" :alt="t('sortableTable.alt.prevPageBtn')" />
      </button>
      <span>
        {{ pagingDisplay }}
      </span>
      <button type="button" class="btn btn-sm role-multi-action" data-testid="pagination-next"
        :disabled="page == totalPages || loading" role="button" :aria-label="t('sortableTable.ariaLabel.nextPageBtn')"
        @click="goToPage('next')">
        <i class="icon icon-chevron-right" :alt="t('sortableTable.alt.nextPageBtn')" />
      </button>
      <button type="button" class="btn btn-sm role-multi-action" data-testid="pagination-last"
        :disabled="page == totalPages || loading" role="button" :aria-label="t('sortableTable.ariaLabel.lastPageBtn')"
        @click="goToPage('last')">
        <i class="icon icon-chevron-end" :alt="t('sortableTable.alt.lastPageBtn')" />
      </button>
    </div>
...
#rounded-table {
  ...
  border: 1px solid rgba(34, 43, 89, 0.63);
