...
<template>
  <header ref="header" data-testid="header">
    ...
    <div class="menu-spacer" :class="{ 'isSingleProduct': isSingleProduct }">
      <router-link v-if="isSingleProduct && !isRancherInHarvester" :to="singleProductLogoRoute" role="link"
      :alt="t('branding.logos.home')">
        <BrandImage v-if="isSingleProduct.supportCustomLogo && isHarvester" class="side-menu-logo"
          file-name="harvester.svg" :support-custom-logo="true" :alt="t('branding.logos.label')" />
        <img v-else class="side-menu-logo" :src="isSingleProduct.logo" :alt="t('branding.logos.label')">
      </router-link>
    </div>
    ...
    <div v-if="!simple" ref="product" class="product">
      <div v-if="currentProduct && currentProduct.showClusterSwitcher" v-clean-tooltip="nameTooltip"
        ...
        <div v-if="isSingleProduct && !isRancherInHarvester" class="product-name">
        ...
        <template v-else>
          ...
          <div v-if="!currentCluster" class="simple-title">
            <BrandImage class="side-menu-logo-img" file-name="rancher-logo.svg" :alt="t('branding.logos.label')" />
          </div>
        </template>
      </div>
      <div v-if="currentProduct && !currentProduct.showClusterSwitcher" class="cluster">
        <img v-if="currentProduct.iconHeader" v-bind="$attrs" :src="currentProduct.iconHeader"
          class="cluster-os-logo mr-10" style="width: 44px; height: 36px;" :alt="t('branding.logos.label')">
    ...
    <div v-else class="simple-title">
      ...
      <div v-else class="side-menu-logo">
        <BrandImage class="side-menu-logo-img" data-testid="header__brand-img" file-name="rancher-logo.svg"
          :alt="t('branding.logos.label')" />
      </div>
    </div>
    <div class="spacer" />
    <div class="rd-header-right">
      ...
      <div v-if="currentCluster && !simple" class="header-buttons">
        <template v-if="currentProduct && currentProduct.showClusterSwitcher">
          <button v-if="showImportYaml" v-clean-tooltip="t('nav.import')" :disabled="!importEnabled" type="button"
            class="btn header-btn role-tertiary" data-testid="header-action-import-yaml" role="button" tabindex="0"
            :aria-label="t('nav.import')" @click="openImport()">
            <i class="icon icon-upload icon-lg" />
          </button>

          <button v-if="showKubeShell" id="btn-kubectl" v-clean-tooltip="t('nav.shellShortcut', { key: shellShortcut })"
            v-shortkey="{ windows: ['ctrl', '`'], mac: ['meta', '`'] }" :disabled="!shellEnabled" type="button"
            class="btn header-btn role-tertiary" role="button" tabindex="0"
            :aria-label="t('nav.shellShortcut', { key: '' })" @shortkey="currentCluster.openShell()"
            @click="currentCluster.openShell()">
            <i class="icon icon-terminal icon-lg" />
          </button>

          <button v-if="showKubeConfig" v-clean-tooltip="t('nav.kubeconfig.download')" :disabled="!kubeConfigEnabled"
            type="button" class="btn header-btn role-tertiary" data-testid="btn-download-kubeconfig" role="button"
            tabindex="0" :aria-label="t('nav.kubeconfig.download')" @click="currentCluster.downloadKubeConfig()">
            <i class="icon icon-file icon-lg" />
          </button>

          <button v-if="showCopyConfig" v-clean-tooltip="t('nav.kubeconfig.copy')" :disabled="!kubeConfigEnabled"
            type="button" class="btn header-btn role-tertiary" data-testid="btn-copy-kubeconfig" role="button"
            tabindex="0" :aria-label="t('nav.kubeconfig.copy')" @click="copyKubeConfig($event)">
            <i v-if="kubeConfigCopying" class="icon icon-checkmark icon-lg" />
            <i v-else class="icon icon-copy icon-lg" />
          </button>
        </template>

        <button v-if="showSearch" id="header-btn-search"
          v-clean-tooltip="t('nav.resourceSearch.toolTip', { key: searchShortcut })"
          v-shortkey="{ windows: ['ctrl', 'k'], mac: ['meta', 'k'] }" type="button" class="btn header-btn role-tertiary"
          data-testid="header-resource-search" role="button" tabindex="0"
          :aria-label="t('nav.resourceSearch.toolTip', { key: '' })" @shortkey="openSearch()" @click="openSearch()">
      ...
      <div v-if="extensionHeaderActions.length" class="header-buttons">
        <button v-for="action, i in extensionHeaderActions" :key="`${action.label}${i}`"
          v-clean-tooltip="handleExtensionTooltip(action)" v-shortkey="action.shortcutKey"
          :disabled="action.enabled ? !action.enabled(ctx) : false" type="button" class="btn header-btn role-tertiary"
          :data-testid="`extension-header-action-${action.labelKey || action.label}`" role="button" tabindex="0"
          :aria-label="action.labelKey ? t(action.labelKey) : action.label"
          @shortkey="handleExtensionAction(action, $event)" @click="handleExtensionAction(action, $event)">
          <IconOrSvg class="icon icon-lg" :icon="action.icon" :src="action.svg"
      ...
      <div class="center-self">
        ...
        <NotificationCenter />
        <rc-dropdown v-if="showUserMenu" :aria-label="t('nav.userMenu.label')">
          <rc-dropdown-trigger ghost small data-testid="nav_header_showUserMenu"
            :aria-label="t('nav.userMenu.button.label')">
            <img v-if="principal && principal.avatarSrc" :src="principal.avatarSrc"
              :class="{ 'avatar-round': principal.roundAvatar }" width="36" height="36" :alt="t('nav.alt.userAvatar')">
          ...
          <template #dropdownCollection>
            ...
            <rc-dropdown-item v-if="showPreferencesLink" @click="$router.push({ name: 'prefs' })">
              {{ t('nav.userMenu.preferences') }}
            </rc-dropdown-item>
            <rc-dropdown-item v-if="showAccountAndApiKeyLink" @click="$router.push({ name: 'account' })">
              {{ t('nav.userMenu.accountAndKeys', {}, true) }}
            </rc-dropdown-item>
            <rc-dropdown-item v-if="authEnabled && shouldShowSloLogoutModal" @click="showSloModal">
              {{ t('nav.userMenu.logOut') }}
            </rc-dropdown-item>
            <rc-dropdown-item v-else-if="authEnabled" @click="$router.push(generateLogoutRoute)">
              {{ t('nav.userMenu.logOut') }}
            </rc-dropdown-item>
          </template>
        </rc-dropdown>
